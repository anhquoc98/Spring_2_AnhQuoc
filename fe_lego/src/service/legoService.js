...
export const findByLego = async (name, page) => {
    try {
        return (await axios.get(`http://localhost:8080/lego/page?name=${name ? name : ''}&page=${page ? page : 0}`)).data
    } catch (e) {
        console.log(e)
    }
}
export const findByListLego = async () => {
    try {
        return (await axios.get(`http://localhost:8080/lego/list`)).data
    } catch (e) {
        console.log(e)
    }
}
