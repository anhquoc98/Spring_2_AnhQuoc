...
public class LegoController {
    ...
    public ResponseEntity<?> findByLego(@PageableDefault Pageable pageable,
        ...
        pageable = PageRequest.of(page, 6, Sort.by("id").descending());
        Page<Lego> legos = legoService.findAllByNameAndPrice(name, pageable);
        return new ResponseEntity<>(legos, HttpStatus.OK);
    }

    @GetMapping("/list")
    public ResponseEntity<?> findByList() {
        return new ResponseEntity<>(legoService.findByList(), HttpStatus.OK);
    }
